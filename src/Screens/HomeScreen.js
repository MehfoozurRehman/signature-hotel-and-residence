...
import buildingfullimg from "../Assets/buildingfullimg.jpg";
...
import strip19 from "../Assets/Image_not_available.png"
import MultiRangeSlider from "multi-range-slider-react";
import Header from "../Components/Header";
import Footer from "../Components/Footer";
import { Link } from "react-router-dom";
import { GetAllUnitsByFilter, GetUnitByMaxRate } from "../Api/building";
...
import { commercial, services, suites, apartments } from "../Constants/constant";
import { getFormattedPrice, getFormattedNumber, priceInThousand } from "../NumberUtils";

export default class HomeScreen extends Component {
    constructor(props) {
        super(props);
        // let categoryId = window.localStorage.getItem("category_id");
        this.state = {
            isScrolling: false,
            ...
            selectedItem: null,
            selectedImageArea: "Visual",
            noImage: strip19,
            showLoader: false
        };
        console.log("floorList", this.state.floorList)
    ...
    fetchUnitWithMaxRate = async () => {
        await GetUnitByMaxRate().then((apiResponse) => {
            if (apiResponse.data)
                if (apiResponse.data.errorCode === 0) {
                    this.setState({
                        maxAreaRate: apiResponse.data.data.unit_area_rate,
                        constantMaxAreRate: apiResponse.data.data.unit_area_rate
                    });
                }
        })
            .catch((err) => {
    ...
    fetchUnitsList = async () => {
        this.setState({ apartments: [], totalArea: "", showLoader: true });
        await GetAllUnitsByFilter(
            this.state.minFloor,
            this.state.maxFloor,
            this.state.minAreaRate,
            this.state.maxAreaRate,
            this.state.unitType
        )
            .then(({ data }) => {
                ...
                if (result) {
                    ...
                    if (result.length > 0) {
                        debugger;
                        this.setState({ apartments: result, selectedItem: result[0].unit_id });
                        this.selectApartment(result[0])
                    }
                    else {
                        this.setState({
                            totalArea: null,
    ...
    selectApartment = (data) => {
        ...
        if (data.storage_list) {
            ...
            floorImage = _.find(data.storage_list, {
                storage_sub_directory: "floor",
            });
            if (floorImage) {

                floorImage = process.env.REACT_APP_BASEURL + floorImage.storage_url;
            }
            planImage = _.find(data.storage_list, { storage_sub_directory: "plan" });

            if (planImage) {

                planImage = process.env.REACT_APP_BASEURL + planImage.storage_url;
            }

            viusalImage = _.find(data.storage_list, { storage_sub_directory: "visual" });

            if (viusalImage) {

                viusalImage = process.env.REACT_APP_BASEURL + viusalImage.storage_url;
            }
        }

        this.setState({
            totalArea: data.unit_area,
            livingArea: data.unit_living_area,
            areaRate: data.unit_area_rate,
            rooms: data.unit_room,
            tower: data.unit_tower,
            floor: data.floor_number,
            price: data.unit_price,
            floorImage: floorImage,
            planImage: planImage,
            viusalImage: viusalImage,
            imageToShow: viusalImage ? viusalImage : this.state.noImage,
            selectedItem: data.unit_id,
            selectedImageArea: "Visual",
            isImageClicked: false
        });
    };
    ...
    setImage = (type) => {
        if (type === 0) {
            this.setState({ imageToShow: this.state.viusalImage ? this.state.viusalImage : this.state.noImage, selectedImageArea: "Visual" });
        } else if (type === 1) {
            this.setState({ imageToShow: this.state.planImage ? this.state.planImage : this.state.noImage, selectedImageArea: "Plan" });
        } else {
            this.setState({ imageToShow: this.state.floorImage ? this.state.floorImage : this.state.noImage, selectedImageArea: "Floor" });
        }
    };
            ...
            <>
                            ...
                            <div className="main__section__container__wrap__links">
                                {this.state.isManagementFloorSelected ? (
                                    <>
                                        <Link
                                            to="/home-commercial"
                                            className="main__section__container__wrap__link"
                                        >
                                            <img
                                                loading="lazy"
                                                src={ring}
                    ...
                    <div className="hotel__services__container__content">
                        <div className="hotel__services__container__content__left">
                            ...
                            <div className="hotel__services__container__content__info">
                                {this.props.selected === commercial ? (
                                    "A BUSINESS CENTER WITH MULTIPLE FACILITIES, WHERE AL L OF YOUR BUSINESS ACTIVITIES AND ERRANDS ARE DONE IN SMOOTH AND EXCEL LENCE AMID AN UNMATCHABLE AURA AND AMBIANCE SURROUND YOU"
                                ) : this.props.selected === suites ? (
                                    <div>
                                        <li
                                            style={{ textTransform: "capitalize", marginBottom: 10 }}
                                        >
                                            <b>Presidential Suites</b>
                                            <br />
                                            <div
                                                style={{ marginLeft: 20, fontSize: 12, marginTop: 10 }}
                                            >
                                                HAVING WHOLESOME OPULENCE AND CLASS, OUR PRESIDENTIAL
                                                SUITES GIVE VIBES OF THE PERFECT BALANCE BETWEEN CHORES
                                                AND COMFORT WITH SPACIOUS AND LUXURIOUS AMBIANCE, THEY
                                                OFFER ENCHANTING VIEWS OF THE BEAUTIFUL CAPITAL
                                                ISLAMABAD
                                            </div>
                                        </li>
                                        <li
                                            style={{ textTransform: "capitalize", marginBottom: 10 }}
                                        >
                                            <b>Executive Suites</b>
                                            <br />
                                            <div
                                                style={{ marginLeft: 20, fontSize: 12, marginTop: 10 }}
                                            >
                                                ENTER INTO A HEAVEN OF SERENITY AND TRANQUIL IN THE
                                                EXECUTIVE ROOM WITH AN APPEALINGLY AND BEAUTIFUL MODERN
                                                DECOR.
                                            </div>
                                        </li>
                                        <li style={{ textTransform: "capitalize" }}>
                                            ...
                                            <div
                                                style={{ marginLeft: 20, fontSize: 12, marginTop: 10 }}
                                            >
                                                SIGNATURE STANDARD ROOM OFFERS SPRUCELY EMBROIDERED
                                                DECOR FUSIONAL WITH MODERN AND ELEGANT DESIGN INCLUDING
                                                FUTURISTIC AMENITIES TO MAKE YOUR STAY A MEMORABLE
                                                EXPERIENCE.
                                            </div>
                                        </li>
                                    </div>
                                    ...
                                    "SHOPPING AURA OF A NEW AGE AND DIMENSION, FULFIL LING THE DIVERSE NEEDS OF IT’S RESIDENTS AND TRAVEL LERS. A RETAIL AREA CONTRIVED AND DESIGNED ON THE INTERNATIONAL STANDARDS AT SIGNATURE HOTELS AND RESIDENCE"
                                ) : null}
                            </div>
                ...
                <div className="apartment__details__container">
                    ...
                    <div className="apartment__details__container__wrapper">
                        <div className="apartment__details__container__wrapper__content">
                            {this.state.isImageClicked && this.state.imageToShow !== this.state.noImage ? (
                                // <Slideshow imageToShow={this.state.imageToShow} />

                                <div className="slide__show__entry">
                                    <img
                                        loading="lazy"
                                        src={this.state.imageToShow}
                                        alt="apartmentPic"
                                        className="slide__show__entry__img"
                                    />
                                    <div className="slide__show__overlay">
                                        <button
                                            className="slide__show__overlay__btn"
                                            onClick={() => {
                                                this.setState({
                                                    isImageClicked: false,
                                                });
                                ...
                                <>
                                    <div className="apartment__details__container__wrapper__content__left">
                                        ...
                                        <div className="apartment__details__container__wrapper__content__left__row">
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    debugger;
                                                    console.log("onchange type", e.currentTarget.value);
                                                    this.setState({
                                                        unitType: e.currentTarget.value === "All Type" ?
                                                            null
                                            ...
                                            >
                                                ...
                                                <option value="Shop" name="Shop">Shops</option>
                                                <option value="Office" name="Office">Offices</option>
                                                <option value="Apartment" name="Apartment" >Apartments</option>
                                            </select>
                                        ...
                                        <div className="apartment__details__container__wrapper__content__left__range">
                                            ...
                                            <MultiRangeSlider
                                                ...
                                                preventWheel={false}
                                                onInput={(e) => {
                                                    this.setState({ minFloor: e.minValue, maxFloor: e.maxValue })
                                                }}
                                            />
                                        ...
                                        <div className="apartment__details__container__wrapper__content__left__range__heading heading">
                                            <label >From&nbsp;&nbsp;</label>
                                            <text>{this.state.minFloor}</text>
                                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                            <label >To&nbsp;&nbsp;</label>
                                            <text>{this.state.maxFloor}</text>
                                        </div><br />
                                        <div className="apartment__details__container__wrapper__content__left__range">
                                            <div className="apartment__details__container__wrapper__content__left__range__heading heading">
                                                Rate/Sq.ft
                                            </div>
                                            {this.state.constantMaxAreRate != 0 ?
                                                <MultiRangeSlider
                                                    min={(0)}
                                                    max={this.state.constantMaxAreRate}
                                                    // minValue={this.state.minAreaRate}
                                                    maxValue={this.state.maxAreaRate}
                                                    step={500}
                                                    ruler={false}
                                                    label={true}
                                                    preventWheel={false}
                                                    onInput={(e) => {
                                                        this.setState({ minAreaRate: e.minValue, maxAreaRate: e.maxValue })
                                                    }}
                                                />
                                                :
                                                null}
                                        ...
                                        <div className="apartment__details__container__wrapper__content__left__range__heading heading">
                                            <label >From&nbsp;&nbsp;</label>
                                            <text>{priceInThousand(this.state.minAreaRate)}</text>
                                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                            <label >To&nbsp;&nbsp;</label>
                                            <text>{priceInThousand(this.state.maxAreaRate)}</text>
                                        </div>
                                        <button
                                            className="heading apartment__details__container__wrapper__content__left__button"
                                            onClick={() => this.fetchUnitByFilter()}
                                        >
                                            View Details
                                        </button>
                                    </div>
                                    <div className="apartment__details__container__wrapper__content__middle">
                                        <div
                                            className="heading"
                                            style={{ marginBottom: ".5em", color: "#ffffff" }}
                                        >
                                            {this.state.apartments.length > 0 ?

                                                'Select To see details'
                                                :
                                                'Sorry! No Record Found against your search'
                                            }
                                        </div>
                                        {this.state.apartments ? (
                                            this.state.apartments.map(
                                                (apartmentObject, apartmentIndex) => (
                                                    <div
                                                        className="apartment__details__container__wrapper__content__middle__entry__wrapper"
                                                        onClick={() =>
                                                            this.selectApartment(apartmentObject)
                                                        }
                                                    >
                                                        <input
                                                            type="radio"
                                                            // defaultChecked={false}
                                                            defaultChecked={this.state.selectedItem == apartmentObject.unit_id ? true : false}
                                                        ...
                                                        <div className="apartment__details__container__wrapper__content__middle__entry">
                                                            <div className="apartment__details__container__wrapper__content__middle__entry__content">
                                                                <div className="apartment__details__container__wrapper__content__middle__entry__content__value">
                                                                    {getFormattedNumber(apartmentObject.unit_area)} m{" "}
                                                                    <sup>2</sup>
                                                                </div>
                                                            ...
                                                            <div className="apartment__details__container__wrapper__content__middle__entry__content">
                                                                <div className="apartment__details__container__wrapper__content__middle__entry__content__value">
                                                                    {apartmentObject.unit_room}
                                                                </div>
                                                                <div className="apartment__details__container__wrapper__content__middle__entry__content__label">
                                                                    Rooms
                                                                </div>
                                                                <div className="apartment__details__container__wrapper__content__middle__entry__content__value">
                                                                    {apartmentObject.floor_number}
                                                                </div>
                                                                <div className="apartment__details__container__wrapper__content__middle__entry__content__label">
                                                                    Floor
                                                                </div>
                                                            ...
                                                            <div className="apartment__details__container__wrapper__content__middle__entry__content">
                                                                ...
                                                                <div className="apartment__details__container__wrapper__content__middle__entry__content__value">
                                                                    {priceInThousand(apartmentObject.unit_area_rate)}
                                                                </div>
                                                                <div className="apartment__details__container__wrapper__content__middle__entry__content__label">
                                                                    Per m <sup>2</sup>
                                                                </div>
                            ...
                            {/* {this.state.totalArea === "" ? (
                                <div className="apartment__details__container__wrapper__content__right">
                                    ...
                                    <div className="apartment__details__container__wrapper__content__right__tabs">
                                        <div
                                            className="apartment__details__container__wrapper__content_right__tabs__entry"
                                            onClick={() => this.setImage(0)}
                                        >
                                            <input
                                                type="radio"
                            ...
                            <div
                                className="apartment__details__container__wrapper__content__right"
                                style={{ width: "100px" }}
                            >
                                <div className="apartment__details__container__wrapper__content__right__heading heading">
                                    Apartments - Comforts
                                ...
                                <button
                                    className="apartment__details__container__wrapper__content__right__img__button"
                                    onClick={() => {
                                        this.setState({
                                            isImageClicked: true,
                                        });
                                ...
                                >
                                    {this.state.imageToShow !== undefined && this.state.imageToShow !== null && this.state.apartments.length > 0 ?
                                        <img
                                            loading="lazy"
                                            src={this.state.imageToShow}
                                            alt="apartmentPic"
                                ...
                                <div className="apartment__details__container__wrapper__content__right__tabs">
                                    ...
                                    >
                                        <input
                                            ...
                                            name="apartment__details__container__wrapper__content_right__tabs__entry__input"
                                            // defaultChecked={true}
                                            checked={this.state.selectedImageArea === "Visual" ? true : false}
                                        />
                                        <div className="apartment__details__container__wrapper__content_right__tabs__entry__content heading">
                                            Visual
                                        </div>
                                    ...
                                    >
                                        <input
                                            ...
                                            className="apartment__details__container__wrapper__content_right__tabs__entry__input"
                                            name="apartment__details__container__wrapper__content_right__tabs__entry__input"
                                            checked={this.state.selectedImageArea === "Plan" ? true : false}
                                        />
                                        <div className="apartment__details__container__wrapper__content_right__tabs__entry__content heading">
                                            Plan
                                        </div>
                                    ...
                                    >
                                        <input
                                            ...
                                            className="apartment__details__container__wrapper__content_right__tabs__entry__input"
                                            name="apartment__details__container__wrapper__content_right__tabs__entry__input"
                                            checked={this.state.selectedImageArea === "Floor" ? true : false}
                                        />
                                ...
                                <div className="apartment__details__container__wrapper__content__right__data__row">
                                    <div className="apartment__details__container__wrapper__content__right__data__row__col">
                                        ...
                                        <div className="apartment__details__container__wrapper__content__right__data__row__value">
                                            {this.state.totalArea ?
                                                getFormattedNumber(this.state.totalArea)
                                                :
                                                '-'}
                                            m <sup>2</sup>
                                        </div>
                                ...
                                <div className="apartment__details__container__wrapper__content__right__data__row">
                                    <div className="apartment__details__container__wrapper__content__right__data__row__col">
                                        ...
                                        <div className="apartment__details__container__wrapper__content__right__data__row__value">
                                            PKR&nbsp;
                                            {this.state.areaRate ?
                                                priceInThousand(this.state.areaRate) :
                                                '-'}
                                        </div>
                                    </div>
                                    <div className="apartment__details__container__wrapper__content__right__data__row__col">
                                ...
                                <div className="apartment__details__container__wrapper__content__right__data__row">
                                    ...
                                    <div className="apartment__details__container__wrapper__content__right__data__row__col">
                                        ...
                                        <div className="apartment__details__container__wrapper__content__right__data__row__value">
                                            PKR&nbsp;
                                            {this.state.price ?
                                                priceInThousand(this.state.price) :
                                                '-'}
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="surrounding__container">
                    <img
                        ...
                        src={ring}
                        alt="ring"
                        className="surrounding__container__img"
                    />
